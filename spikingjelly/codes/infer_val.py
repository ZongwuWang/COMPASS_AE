...
import models
import math
from hooks import register_hook, unregister_hook
...
if __name__ == '__main__':
    ...
    dataset_dir = '/root/ReRAM_SNN_Acce/codes/datasets/' + dataset_name
    ...
    if os.path.exists(check_point_path):
        ...
        assert False, "Trained model is not found"
    ...
    assert check_point != None, "Can not checkpoint"
    try:
        ...
        # log_data_list = []
        print("Failed to checkpoint trained model")
    ...
    net.eval()
    # 注册钩子
    hook_lists = register_hook(net, args.dataset_name)
    with torch.no_grad():
        test_sum = 0
        correct_sum = 0
