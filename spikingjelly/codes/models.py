...
# from spikingjelly.activation_based import functional, layer, surrogate, accelerating
...
def create_2fc(channels, h, w, dpp, class_num, init_tau, use_plif, alpha_learnable, detach_reset):
    return nn.Sequential(
        ...
        # surrogate.ATan在6fbe4983fcbf7dd8759b1ffbc73745e7d5ee83ca去除了可学习参数，参考https://github.com/fangwei123456/spikingjelly/issues/8
        PLIFNode(init_tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset) if use_plif else LIFNode(tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset),
        layer.Dropout(dpp),
        # the out features of the second FC layer is M · C, where C is the classes number and M is the neurons of a population to represent one class
...
class CIFAR10DVSNet(NeuromorphicNet):
    def __init__(self, T, init_tau, use_plif, use_max_pool, alpha_learnable, channels, number_layer, detach_reset, use_ptb_model=False):
        super().__init__(T=T, init_tau=init_tau, use_plif=use_plif, use_max_pool=use_max_pool, alpha_learnable=alpha_learnable, detach_reset=detach_reset)
        w = 128
        h = 128
        if use_ptb_model:
            channel_list = [2, 128, 128, 128, 256]
            pool_list = [False, True, False, True]
            conv = [
                nn.Conv2d(in_channels=channel_list[0], out_channels=channel_list[1], kernel_size=3, padding=0, bias=False), 
                nn.BatchNorm2d(channel_list[1]),
                PLIFNode(init_tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset) if use_plif else LIFNode(tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset),
                # nn.MaxPool2d(2, 2) if use_max_pool else nn.AvgPool2d(2, 2)
            ]
            for idx, chan in enumerate(channel_list[1:-1]):
                conv.extend([
                    nn.Conv2d(in_channels=chan, out_channels=channel_list[idx+2], kernel_size=3, padding=1, bias=False), 
                    nn.BatchNorm2d(channel_list[idx+2]),
                    PLIFNode(init_tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset) if use_plif else LIFNode(tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset),
                ])
                if pool_list[idx + 1]:
                    nn.MaxPool2d(2, 2) if use_max_pool else nn.AvgPool2d(2, 2)
            ...
            self.fc = nn.Sequential(
                nn.Flatten(),
                layer.Dropout(0.5),
                nn.Linear(channel_list[-1] * 10 * 10, 1024 * 1 * 1, bias=False),
                PLIFNode(init_tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset) if use_plif else LIFNode(tau=init_tau, surrogate_function=surrogate.ATan(), detach_reset=detach_reset),
                layer.Dropout(0.5),
                nn.Linear(1024 * 1 * 1, 10 * 10, bias=False),
            )
        else:
