...
def plif_hook_fn(layer_name):
    def plif_hook(module, input, output):
        ...
        mp_fname = './layer_record_' + str(dataset) + '/mp.' + full_name + '.csv'
        f = open('./PyNeuroSim/layer_record_'+str(dataset)+'/trace_command.sh', "a")
        fwrite = f.write(output_fname + ' ' + mp_fname + ' ')
        f.close()
        # TODO:
        if 'fc' in full_name:
            for item in output.cpu().data.numpy():
                write_matrix_output_fc(item, None, 1, os.path.join(filePrefix, output_fname))
            for item in module.v_seq.cpu().data.numpy():
                write_matrix_output_fc(item, None, 1, os.path.join(filePrefix, mp_fname))
        else:
            for item in output.cpu().data.numpy():
                write_matrix_output_conv(item, None, 1, os.path.join(filePrefix, output_fname))
            for item in module.v_seq.cpu().data.numpy():
                write_matrix_output_conv(item, None, 1, os.path.join(filePrefix, mp_fname))
    return plif_hook
