...
def main():
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    for layer in net.layer:
        if isinstance(layer, neuron.LIFNode):
            lif_layers.append(layer)
            layer.v_seq = []
            layer.s_seq = []
    ...
    with torch.no_grad():
        ...
        for idx, layer in enumerate(lif_layers):
            layer.v_seq = torch.cat(layer.v_seq)
            layer.s_seq = torch.cat(layer.s_seq)
            np.save(f"layer{idx}_v_t_array.npy",layer.v_seq.cpu().numpy().squeeze())
            np.save(f"layer{idx}_s_t_array.npy",layer.s_seq.cpu().numpy().squeeze())
