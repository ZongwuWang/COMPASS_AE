...
class Trainer:
    ...
    def evaluate(self, args, model, criterion, data_loader, device, log_suffix=""):
        print(f"Start evaluating {log_suffix}...")
        print(model)
        # breakpoint()
        if args.hook:
            # 注册钩子
            ...
            # 为模型的所有子模块添加hooks
            handles = register_hook(model, 'IMAGENET', [])
        ...
        with torch.inference_mode():
            for image, target in tqdm(metric_logger.log_every(data_loader, -1, header)):
    ...
    def main(self, args):
        ...
        if args.hook:
            ...
            data_loader_test = torch.utils.data.DataLoader(
                dataset_test, batch_size=1, sampler=test_sampler, num_workers=args.workers, pin_memory=not args.disable_pinmemory,
                worker_init_fn=seed_worker
            )
        else:
            data_loader_test = torch.utils.data.DataLoader(
        ...
        if args.resume is not None:
            print(f"Reusming from pt_dir={pt_dir}")
...
# python3 -m spikingjelly.activation_based.model.train_imagenet_example --device cuda:1 --T 100 --model spiking_resnet18 --data-path /root/ReRAM_SNN_Acce/codes/datasets/imagenet/ --batch-size 1 --lr 0.1 --lr-scheduler cosa --epochs 90 --output-dir inferlogs --test-only
